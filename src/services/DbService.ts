...
export class DbService {
  ...
  async create(filePath: string, fileName: string, defaultData: IDbServiceScheme) {
  ...
  async getData(): Promise<IDbServiceScheme> {
    return JSON.parse(await readFile(DbService.fullFilePath, 'utf-8'));
  }

  async setData(payload: IDbServiceScheme): Promise<void> {
